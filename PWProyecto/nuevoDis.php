...
function validar($nombre, $apellido, $telefono, $domicilio, $correo, $contraseña1)
{
    global $error;
    global $mensaje;
    $error = 0;
...
if (isset($datos_request['pass']) && isset($datos_request['correo'])) {
    ...
    validar($nombre, $apellido, $telefono, $domicilio, $correo, $contraseña1);
        ...
        if (is_null($conn)) {
            ...
            if (($count1 == 0) && ($count2 == 0)) {
                try {
                    ...
                    $stmt = $conn->prepare("SELECT * from distribuidores");
                    $stmt->setFetchMode(PDO::FETCH_ASSOC);
                    $stmt->execute();
                    $count = $stmt->rowCount();
                    ...
                    $Re_cliente = "INSERT INTO distribuidores (IdD, Nombre, Apellido, Correo, Tel, Dom, Activo) 
                    VALUES (:IdD, :Nombre, :Apellido, :Correo, :Tel, :Dom,0)";
                    $stmt = $conn->prepare($Re_cliente);

                    $stmt->bindParam(':IdD', $count);
